...
int main() {
	...
	while (runProgram) {
		...
		if (funkcija == 5) {
			...
			SetConsoleTextAttribute(h, 12);
			std::cout << "\nLaikas pateikiamas sekundemis!" << std::endl;
			SetConsoleTextAttribute(h, 15);
			std::cout << std::left << std::setw(25) << "Irasu skaicius faile" << std::left << std::setw(20) << "find_if + move" << std::left << std::setw(20) << "stable_partition" << std::left << std::setw(20) << "find_if + 2x move" << std::left << std::setw(20) << "partition_copy" << std::endl;
